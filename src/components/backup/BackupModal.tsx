...
export function BackupModal({ data, onClose, onMerge, onReplace }: BackupModalProps) {
  ...
  const handleReplace = () => {
    const imported = parseValue()
    if (!imported) return
    if (!window.confirm("Replace all current progress with this backup? This cannot be undone."))
      return
    onReplace(imported)
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Copy your backup string or paste one to restore progress.
            </p>
          </div>
          <button
            type="button"
        ...
        >
          ...
          {copied ? "Copied" : "Copy backup string"}
        </button>
